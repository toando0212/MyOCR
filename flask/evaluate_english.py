...
# Suppress the specific scikit-learn warning about feature names
...
# Suppress INFO logs from our custom pipeline module as well
logging.getLogger('english_pipeline').setLevel(logging.WARNING)
...
def evaluate_pipeline():
    ...
    for filename in tqdm(image_files, desc="Evaluating"):
        ...
        try:
            ...
            if predicted_text and isinstance(predicted_text, str):
                # The new pipeline returns clean text, so we just need to join the lines.
    ...
    if not results:
        print("Evaluation could not be completed for any image.")
        return
